...
// 		uint16 _quorom
// 	) DaoVault(_vaultToken, name, symbol) {
...
// 		// 1 day < time < 3 days
// 		require(time >= 86400 && time <= 259200 && length <= 3);
...
// 	) external {
// 		require(
// 			id < proposals[descriptor].idSnapshot && msg.sender == ownerOf[id]
// 		);
...
// 			// no vote
// 			proposals[descriptor].none +=
// 				delegatedAmount[id] +
// 				(deposits[id].amount - delegation[id].weight);
// 		} else {
// 			votes[descriptor][key] +=
// 				delegatedAmount[id] +
// 				(deposits[id].amount - delegation[id].weight);
// 		}
...
// 		require(proposal.endTime <= block.timestamp && !proposal.executed);
...
// 		require(
// 			totalVotes >= (totalDeposits * ctx.quorom) / 1e4,
// 			"Not Enough Votes"
// 		);
...
// 		require(msg.sender == ownerOf[fromId] && fromId != toId);
